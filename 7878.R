...
cor(kor,eng) # ?紐닺퉪紐꾧맒?꽴??④쑴?땾
cor.test(kor,eng) # ?湲썸꽴??겫袁⑷퐤
...
t0 <- (car_lm$coefficients[2]-(-0.8))/(sqrt(anova(car_lm)["Residuals","Mean Sq"]/sxx)) ###[행, 열]
t0; t0 <= qt(0.05,10) # ??꽰?④쑬?쎗??뵠 疫꿸퀗而??肉??肉? ?諭??堉긷첎?筌왖 筌륁궢釉???뼄.
#qt는 기각ㅇ
confint(car_lm)
...
plot(rstandard(car_lm)) # ??뮞??뮔??쐲??뱜??넅 ??삆筌△뫀猷?
abline(h=0) # ??땾?猷??苑?
...
iris_lm2 <- lm(Sepal.Width~., data=iris) # 筌뤴뫀諭? ?苑뺧쭗?굝????땾?몴? ??뿯??젾
summary(iris_lm2)
